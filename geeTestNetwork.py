...
class SiamDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img1, img2 = cv2.imread(f"trainData/icons/{path_img1}", cv2.IMREAD_UNCHANGED), cv2.imread(f"trainData/{path_img2}")
        ...
        img1, img2 = cv2.cvtColor(img1, cv2.COLOR_BGRA2RGB), cv2.cvtColor(img2, cv2.COLOR_BGR2RGB)
...
iconNames = os.listdir("trainData/icons")
for i in range(len(iconNames)):
    ...
    for k in range(len(iconNames)):
        ...
        for data in os.listdir(f"trainData/{name}"):
            if i == k:
                pos_pairs.append([iconNames[i], f"{name}/{data}"])
            else:
                neg_pairs.append([iconNames[i], f"{name}/{data}"])
...
for epoch in range(num_epochs):
    ...
    imgPairs = pos_pairs + neg_sampled
    labels = [0] * len(pos_pairs) + [1] * len(neg_sampled)
